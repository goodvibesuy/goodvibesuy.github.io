...
export class DetalleLocalComponent extends GeneralComponent implements OnInit {
    private params: any;
    private id: string;
    ...
    getPointOfSale(id: Number): void {
        var pos: PointOfSale;
        this.pointOFSaleService.getPointOfSale(this.generateHeader(), id).subscribe(
            result => {
                console.log()
    ...
    agregar():void{
        this.viewingService.addViewing(this.generateHeader(),this.pointOfSale.id,this.productsToSend).subscribe(
            response => {
