...
class PointOfSaleController extends MainController {
    ...
    public add = (req: any, res: any): void => {
        this.verifyAccess(req, res, this.resource,
            (dbName: string) => {
                this.pointsOfSaleModel.add(req.body.name, req.body.businessName,req.body.contactName,req.body.RUT,req.body.group,req.body.address, req.body.tel, req.body.image, req.body.coord, dbName, function (result: any) {
                    res.send(result);
                });
            }
        );
